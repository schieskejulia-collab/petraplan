...
test("complete Northwind order runs through source adapter and bridge without inventing semantics", () => {
  ...
  assert.deepEqual(adaptation.issues.map(({ code }) => code), ["NO_CONFIRMED_SEMANTIC_MAPPING"]);
  ...
  // Confirmed mappings survive the full bridge run.
  ...
  // The unconfirmed status becomes a blocker; release must stay closed.
  assert.equal(evaluation.mapped.status, null);
  assert.equal(evaluation.release.releaseAllowed, false);
  assert.ok(evaluation.release.blockingIssues >= 1);
  assert.ok(evaluation.report.errors.length >= 1);
