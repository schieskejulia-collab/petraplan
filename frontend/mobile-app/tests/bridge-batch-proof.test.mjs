...
const records = [
  {
    raw: { KUNDEN_NR: "4711", AUFTRAGS_NR: "A-10027", STATUS: "OFFEN", MENGE: "12", DATUM: "2026-09-19" },
    capturedAt,
    ingress: ingress("valid-1"),
  },
  {
    raw: { KUNDEN_NR: "8150", AUFTRAGS_NR: "A-10027", STATUS: "GESCHLOSSEN", MENGE: "3", DATUM: "19.09.2026" },
    capturedAt,
    ingress: ingress("valid-2"),
  },
  {
    raw: { KUNDEN_NR: "9001", AUFTRAGS_NR: "A-10027", STATUS: "A", MENGE: "5", DATUM: "2026-09-19" },
    capturedAt,
    ingress: ingress("ambiguous-status"),
  },
  {
    raw: { KUNDEN_NR: "ALFKI", AUFTRAGS_NR: "A-10027", STATUS: "OFFEN", MENGE: "9", DATUM: "2026-09-19" },
    capturedAt,
    ingress: ingress("customer-conflict"),
    conflicts: [{
      field: "KUNDEN_NR",
  ...
  {
    raw: { KUNDEN_NR: "7777", AUFTRAGS_NR: "A-10027", STATUS: "IN_BEARBEITUNG", MENGE: "-4", DATUM: "2026-09-19" },
    capturedAt,
    ingress: ingress("negative-quantity"),
  },
];

test("mixed five-record batch releases valid siblings and blocks only failing records", () => {
  ...
  assert.deepEqual(records, before, "Batch evaluation must preserve caller-owned Source Truth.");
  ...
  assert.equal(valid1.state, "VALID");
  assert.equal(valid2.state, "VALID");

  // Technically valid syntax, but no confirmed meaning: do not guess.
  ...
  assert.ok(ambiguous.evaluation.constraints.some(({ id, passed }) => id === "status.value_map" && passed === false));

  // Source-only contradiction reaches Conflict Truth, field trace and report.
  ...
  assert.ok(mismatch.evaluation.report.errorsDetailed.some(({ origin, code }) =>
    origin === "adapter" && code === "adapter.CUSTOMER_MISMATCH:KUNDEN_NR"
  ));

  // Formal number, fachlich invalid: quantity rule blocks only this record.
  assert.equal(negative.evaluation.mapped.quantity, -4);
  assert.equal(negative.releaseAllowed, false);
  assert.ok(negative.evaluation.constraints.some(({ id, passed }) => id === "quantity.positive" && passed === false));

  // One bad sibling must never contaminate a valid sibling's decision.
