  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-3xl px-4 py-5 pb-16">
        ...
        {registeredCandidates.length > 0 && <section className="mb-4 rounded-2xl border bg-card p-4 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Adressschicht · gespeicherter Snapshot</p><h2 className="mt-1 text-lg font-semibold">Auflösbare Kandidaten</h2><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Quelle und Auswirkungen sind verlinkt. Ein Kandidat bleibt unwirksam, bis eine fachliche Entscheidung dokumentiert ist.</p><div className="mt-3 space-y-3">{registeredCandidates.map((candidate) => { const source = addressById.get(String(candidate.source_address_id)); const impacts = (linksByCandidateId.get(String(candidate.id)) ?? []).map((link) => addressById.get(String(link.address_id))).filter(isRecord); const history = historyByCandidateId.get(String(candidate.id)) ?? []; return <div key={String(candidate.id)} className="rounded-xl border p-3 text-xs"><div className="flex items-start justify-between gap-2"><strong className="break-all font-mono">{String(candidate.candidate_key)}</strong><span className="shrink-0 rounded-full border px-2 py-1 text-[10px] font-semibold">{String(candidate.state).toUpperCase()}</span></div><p className="mt-2">{representationValue(candidate.observed_value)} → {representationValue(candidate.proposed_value)}</p><p className="mt-1 leading-relaxed text-muted-foreground">{String(candidate.evidence)}</p>{source && <p className="mt-2"><span className="text-muted-foreground">Quelle: </span><a className="break-all underline" href={`#${addressFragment(source.address)}`}>{String(source.address)}</a></p>}<p className="mt-2 text-muted-foreground">Auswirkungen: {impacts.length ? impacts.map((address, index) => <span key={String(address.id)}>{index > 0 && ', '}<a className="break-all underline" href={`#${addressFragment(address.address)}`}>{String(address.address)}</a></span>) : '—'}</p><details className="mt-2"><summary className="cursor-pointer font-medium">Zustandshistorie ({history.length})</summary><div className="mt-2 space-y-2">{history.map((entry) => <div key={String(entry.id)} className="rounded-lg bg-muted/50 p-2"><strong>{String(entry.state).toUpperCase()}</strong><span className="text-muted-foreground"> · {String(entry.changed_by)} · {String(entry.changed_at)}</span><p className="mt-1 text-muted-foreground">{String(entry.reason)}</p></div>)}</div></details></div>; })}</div><details className="mt-4"><summary className="cursor-pointer text-sm font-semibold">{registeredAddresses.length} registrierte Adressen</summary><div className="mt-2 space-y-2">{registeredAddresses.map((address) => <div key={String(address.id)} id={addressFragment(address.address)} className="rounded-lg border p-2 text-xs"><strong className="break-all font-mono">{String(address.address)}</strong><p className="mt-1 text-muted-foreground">{String(address.kind)} · {String(address.source_path)}</p></div>)}</div></details></section>}
