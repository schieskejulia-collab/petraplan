...
function releaseLabel(status: CaseListItem["release_status"]) {
  if (status === "trusted") return "FREIGEGEBEN";
  if (status === "blocked") return "BLOCKIERT";
  if (status === "revoked") return "WIDERRUFEN";
  if (status === "superseded") return "ERSETZT";
  if (status === "exception") return "AUSNAHME";
  return "OFFEN";
}

function badge(status: CaseListItem["release_status"]) {
  return <span className="rounded-full border px-2 py-1 text-[11px] font-semibold">{releaseLabel(status)}</span>;
}
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-3xl space-y-5 px-4 py-6">
        <header className="space-y-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Gespeicherte Fälle aus Quelle, Übersetzung, Prüfung und Freigabe.</p>
            </div>
        ...
        {!loading && !error && items.length === 0 && (
          <div className="rounded-xl border p-6 text-sm text-muted-foreground">Noch keine gespeicherten Live-Fälle vorhanden.</div>
        )}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">{item.source_system ?? "Quelle unbekannt"} · {item.category}</p>
                  <p className="mt-2 text-xs text-muted-foreground">{item.conflict_count} offener Konflikt{item.conflict_count === 1 ? "" : "e"}</p>
                </div>
