...
export function evaluateConstraintSet(input: ConstraintInput): ConstraintResult[] {
  ...
  const constraints: ConstraintResult[] = [
    ...
    result(input, {
      ...
      resolutionProposal: typeSchema.length === 0 ? "Keine Typauflösung nötig." : `Typen für ${typeSchema.map(({ field }) => field).join(", ")} gegen ${input.contract.name} prüfen.`,
    }),
    result(input, {
      ...
      resolutionProposal: formatSchema.length === 0 ? "Keine Formatauflösung nötig." : `Formate für ${formatSchema.map(({ field }) => field).join(", ")} gegen ${input.contract.name} prüfen.`,
    }),
    result(input, {
      id: "contract.schema", sequence: 3.3, category: "contract", field: null,
      label: "Legacy-Schema-Gesamtsicht", passed: failedSchema.length === 0, severity: "warning",
      comparison: { operator: "schema_matches", expected: `${input.contract.name}: Vollständigkeit + Typ + Format`, observed: failedSchema.length === 0 ? "schemaFehler=0" : failedSchema.map(({ field }) => field).join(","), observedType: "schema-summary" },
      rule: "Kompatibilitäts-Summary; Freigabe wird durch contract.completeness, contract.type und contract.format entschieden",
      evidence: failedSchema.length === 0 ? "schemaFehler=0" : `schemaFehler=${failedSchema.length}; felder=${failedSchema.map(({ field }) => field).join(",")}`,
      safeAction: "Für Ursachen immer die drei granularen Contract-Constraints verwenden.",
      resolutionProposal: "Legacy-Summary nicht separat auflösen; die granularen Contract-Constraints bearbeiten.",
    }),
    result(input, {
      id: "customer.required", sequence: 4, category: "data", field: "KUNDEN_NR",
      label: "Kundenkennung ist vorhanden", passed: input.checks.find(({ field }) => field === "KUNDEN_NR")?.ok ?? false, severity: "blocking",
      comparison: { operator: "not_missing", expected: "nicht leer / nicht NULL / nicht N/A", observed: input.raw.KUNDEN_NR || "<leer>", observedType: typeof input.raw.KUNDEN_NR },
      rule: "KUNDEN_NR ist ein Pflichtfeld", evidence: `KUNDEN_NR=${input.raw.KUNDEN_NR || "<leer>"}`,
      safeAction: "Fehlende Kundenkennung nicht erfinden.", resolutionProposal: "Kundenkennung an der Quelle ergänzen oder die Pflichtregel fachlich neu bestätigen.",
    }),
    result(input, {
      id: "status.value_map", sequence: 6, category: "semantics", field: "STATUS",
      label: "Statuswert hat eine bestätigte Bedeutung", passed: input.mapped.status !== null, severity: "blocking",
      comparison: { operator: "in", expected: statusSources.join(" | "), observed: input.raw.STATUS, observedType: typeof input.raw.STATUS },
      ...
      resolutionProposal: input.mapped.status !== null ? `Bestätigte Zuordnung verwenden: ${input.raw.STATUS} → ${input.mapped.status}.` : `Neue Value-Map für STATUS=${input.raw.STATUS} fachlich bestätigen. Mögliche Zielwerte sind open, closed oder in_progress; die Bridge wählt keinen davon eigenmächtig.`,
    }),
    result(input, {
      id: "quantity.positive", sequence: 7, category: "data", field: "MENGE",
      label: "Menge erfüllt die bestätigte Fachregel", passed: input.checks.find(({ field }) => field === "MENGE")?.ok ?? false, severity: "blocking",
      comparison: { operator: "greater_than", expected: "0", observed: quantity === null ? input.raw.MENGE : String(quantity), observedType: quantity === null ? typeof input.raw.MENGE : typeof quantity },
    ...
    result(input, {
      id: "date.canonical", sequence: 8, category: "data", field: "DATUM",
      label: "Datum ist eindeutig normalisiert und kalendergültig", passed: dateCheckPassed && calendarDatePassed, severity: "blocking",
      comparison: { operator: "matches", expected: "existierendes Kalenderdatum im Format YYYY-MM-DD", observed: input.mapped.orderDate ?? "<nicht übernommen>", observedType: input.mapped.orderDate === null ? "unresolved" : typeof input.mapped.orderDate },
      ...
      resolutionProposal: dateCheckPassed && calendarDatePassed ? "Keine Datumsauflösung nötig." : "Datumsformat und tatsächliche Kalendergültigkeit an der Quelle prüfen; keinen unmöglichen Tag oder Monat automatisch korrigieren.",
    }),
  ];

  if (input.ingress.source === "system_a") {
    constraints.push(result(input, {
      id: "order.demo_reference", sequence: 5, category: "data", field: "AUFTRAGS_NR",
      label: "Auftragsnummer entspricht dem Demo-Referenzfall", passed: input.checks.find(({ field }) => field === "AUFTRAGS_NR")?.ok ?? false, severity: "warning",
      comparison: { operator: "strict_equals", expected: "A-10027", observed: input.raw.AUFTRAGS_NR, observedType: typeof input.raw.AUFTRAGS_NR },
      rule: "Demo-Referenz ist A-10027", evidence: `AUFTRAGS_NR=${input.raw.AUFTRAGS_NR}`,
  ...
  if (unknownFields.length > 0) {
    constraints.push(result(input, {
      id: "contract.unknown_fields", sequence: 9, category: "contract", field: null,
      label: "Zusätzliche Quellfelder sind nicht Teil des bestätigten Vertrags", passed: false, severity: "warning",
      comparison: { operator: "schema_matches", expected: `nur bestätigte Felder aus ${input.contract.name}`, observed: unknownFields.join(", "), observedType: "unknown-source-fields" },
